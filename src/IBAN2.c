...
/*
 ...
 Kurzbeschreibung der Aufgabe der Funktion:   Bankleitzahlen und Filialbezeichnungen aus  BLZ.txt einlesen
 und die Bankleitzahlen und Filialbezeichnungen zeilenweise auf dem Bildschirm ausgeben
...
/*
 ...
 Eingaben  : char IBAN , int a
 Ausgaben  : ergebnis ( = IBAN % a )

 Autor     : Hamza Sliti

 Quelle : https://www.geeksforgeeks.org/how-to-compute-mod-of-a-big-number/
...
/*
 ...
 Ausgaben  : 1 (BLZnummer ist richtig )
...
int BLZpruefer(char* BLZnummer) {
	...
	if (laenge == 8) {
		for (i = laenge - 1; i >= 0; i--) {
			...
			if (BLZnummer[i] < '0' || BLZnummer[i] > '9') {   //
				return (-1);
			}
...
int Filialepruefer(char* BLZnummer) {
	...
	FILE *fp = NULL; // File pointer deklarieren
	int Suchergebnis = 0;
	char Linie[50];
	...
	while (fgets(Linie, 50, fp) != NULL && Suchergebnis <= 0) // Text aus der Datei  lesen und als String  speichern
	{
...
/*
 ...
 Eingaben  : kontonummer

 Ausgaben  : 1 (kontonummer ist richtig )
...
/*
 ...
 Eingaben  : kontonummer , neuKontonummer
...
int main(void) {
	char BLZ[8] = { '0' };  // BLZ Array mit 8 char-Elementen
	char Kontonummer[10] = { '0' }; // Kontonummer Array mit 10 char-Elementen
	char IBAN[24] = { '0' };  // IBAN Array mit 24 char-Elementen
	char DE[6] = { "131400" }; // DE Array mit 6 char-Elementen Ländercode für Deutschland + Prüfziffer
	char neuKontoNummer[10] = { '0' }; // neuKontonummer Array mit 10 char-Elementen
	int Kontogueltig = 0, BLZgueltig = 0, Filialegueltig = 0;
	int laenge = 0;  //Länge der Kontonummer
	int Rest = 0; //
	int PP = 0; // Prüfziffer
	...
	while (1) {
		...
		// wenn die Länge der Kontonummer < 10
		if (laenge < 10) {
		...
		printf("\nBLZ\t\t\tKontonummer\t\t\t nIBAN\n%s\t\t%s\t\t %s%s%s\n", BLZ,
				Kontonummer, BLZ, Kontonummer, DE); //BLZ , Kontonummer und nIBAN  ausgeben
